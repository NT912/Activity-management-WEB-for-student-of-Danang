...
activityController.apiAddActivity = async (req, res) => {
  try {
    const userss = req.session.user;
    if (!userss || req.session.user.role == roles.STUDENT) {
      return res
        .status(403)
        .json({ error: "Bạn không được phép tạo hoạt động" });
    }
    ...
    if (
      ...
      return res.status(400).json({ error: "Vui lòng nhập đủ các trường" });
    }
    ...
    if (
      ...
      if (url) {
        ...
        if (!activity) {
          return res
            .status(500)
            .json({ error: "Có lỗi xảy ra khi thêm hoạt động" });
        }
        return res
          .status(201)
          .json({
            message: `Bạn đã đăng ký thành công hoạt động ${req.body.name}`,
            activity,
          });
      } else {
        return res.status(500).json({ error: "fail firebase" });
      }
    } else {
      return res.status(400).json({ error: "Nhập ngày phù hợp" });
    }
  } catch (error) {
    console.error(error);
...
activityController.apiGetActivities = async (req, res) => {
  try {
    const activities = await activityModel.getAll();
...
activityController.Post_register = async (req, res) => {
  try {
    ...
    const updateresult = await studentModel.updateContactInfo(
      userss.id,
      email,
      phone_number
    );
