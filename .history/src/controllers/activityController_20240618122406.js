...
activityController.Post_register = async (req, res) => {
  try {
    ...
    if (!userss) {
      return res.json({
        success: false,
        message: "Bạn cần đăng nhập để đăng ký hoạt động",
      });
    }
    ...
    if (userss.role !== roles.STUDENT) {
      return res.json({
        success: false,
        message: "Bạn không được phép đăng ký tham gia hoạt động",
      });
    }
    ...
    if (!activity) {
      return res.json({ success: false, message: "Hoạt động không tồn tại" });
    }
    ...
    if (
      ...
      return res.json({
        success: false,
        message: "Đã hết thời gian đăng ký hoạt động",
      });
    }
    ...
    if (isRegistered) {
      return res.json({
        success: false,
        message: "Bạn đã đăng ký hoạt động này trước đó",
      });
    }
    ...
    if (!email || !phone_number) {
      return res.json({
        success: false,
        message: "Vui lòng nhập đầy đủ thông tin email và số điện thoại",
      });
    }
    ...
    if (!result) {
      return res.json({
        success: false,
        message: "Có lỗi xảy ra khi đăng ký hoạt động",
      });
    }

    return res.json({
      success: true,
      message: `Bạn đã đăng ký tham gia hoạt động ${activity.name} thành công`,
    });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
      .json({ success: false, message: "Có lỗi xảy ra khi xử lý yêu cầu" });
  }
};
...
activityController.unregister = async (req, res) => {
  try {
    ...
    if (!userss) {
      return res.json({
        success: false,
        message: "Bạn cần đăng nhập để hủy đăng ký hoạt động",
      });
    }
    ...
    if (userss.role !== roles.STUDENT) {
      return res.json({
        success: false,
        message: "Bạn không được phép hủy đăng ký tham gia hoạt động",
      });
    }
    ...
    if (
      ...
      return res.json({
        success: false,
        message: "Đã hết thời gian hủy đăng ký hoạt động",
      });
    }
    ...
    if (!isRegistered) {
      return res.json({
        success: false,
        message: "Bạn không thể hủy đăng ký khi chưa đăng ký",
      });
    }
    ...
    if (!result) {
      return res.json({
        success: false,
        message: "Có lỗi xảy ra khi hủy đăng ký hoạt động",
      });
    }

    return res.json({
      success: true,
      message: `Hủy đăng ký hoạt động ${activity.name} thành công`,
    });
  } catch (error) {
    console.error(error);
    return res
