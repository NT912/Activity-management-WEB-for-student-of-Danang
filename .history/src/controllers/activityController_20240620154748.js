...
activityController.Post_register = async (req, res) => {
  try {
    ...
    if (userss.role !== roles.STUDENT) {
      req.flash("announc", "Bạn không được phép đăng ký tham gia hoạt động");
      return res.redirect(`/activity/${activity_id}/view`);
    }
    ...
    if (!activity) {
      throw new Error("Hoạt động đã bị xóa");
    }
    ...
    const student = await studentModel.getSvRegister(userss.id);
    ...
    if (isRegistered) {
      throw new Error("Bạn đã đăng ký hoạt động này trước đó");
    }

    // Lấy thông tin từ req.body
    const { email, phone_number, wish } = req.body;

    // Kiểm tra và cập nhật thông tin email và số điện thoại nếu không có trong DB
    if (!student.email || !student.phone) {
      if (!email || !phone_number) {
        throw new Error(
          "Vui lòng nhập đầy đủ thông tin email và số điện thoại"
        );
      }
    ...
    const result = await activityModel.register(
      activity_id,
      userss.id,
      email || student.email, // Sử dụng email từ req.body hoặc từ DB nếu đã có
      phone_number || student.phone, // Sử dụng phone_number từ req.body hoặc từ DB nếu đã có
      wish
    );
    ...
    // Thông báo đăng ký thành công và chuyển hướng đến trang xem chi tiết hoạt động
    req.flash(
      "announc",
      `Bạn đã đăng ký tham gia hoạt động ${activity.name} thành công`
    );
    return res.redirect(`/activity/${activity_id}/view`);
  } catch (error) {
    // Xử lý lỗi và hiển thị lại trang xem chi tiết hoạt động với thông báo lỗi
    req.flash("announc", error.message);
    const student = await studentModel.getSvRegister(req.session.user.id);
    return res.render("activity/view", {
      error: error,
      student: student,
    });
  }
