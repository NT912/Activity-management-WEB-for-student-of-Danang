...
activityController.getRegisterModal = async (req, res) => {
  try {
    const activityId = req.params.activity_id;
    // Đối với mục đích minh họa, chúng ta sẽ chỉ trả về một dòng văn bản cho nội dung modal
    const modalContent = "Nội dung modal hiển thị ở đây";
    res.send(modalContent);
  } catch (error) {
    console.error("Error while getting register modal:", error);
