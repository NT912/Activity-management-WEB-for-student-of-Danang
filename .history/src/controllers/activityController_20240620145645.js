...
activityController.Post_register = async (req, res) => {
  try {
    ...
    let { email, phone_number, wish } = req.body;

    // Kiểm tra và cập nhật thông tin email và số điện thoại nếu không có trong database
    if (!student.email || !student.phone) {
      ...
      // Sử dụng thông tin đã có nếu đã tồn tại trong database
      email = student.email;
      phone_number = student.phone;
    }

    // Thực hiện đăng ký hoạt động
    ...
    // Redirect về trang view hoạt động với thông báo thành công
    return res.redirect(`/activity/${activity_id}/view`);
  } catch (error) {
    req.flash("announc", error.message);
    ...
    return res.render("activity/register", {
      error: error,
      student: student,
      activity: await activityModel.GetById(req.params.activity_id),
    });
  }
};
