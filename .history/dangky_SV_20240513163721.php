...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($hoten) || empty($masv) || empty($matkhau) || empty($lop) || empty($major_name)) {
        ...
        if ($result_check->num_rows > 0) {
            ...
            $hashed_password = password_hash($password, PASSWORD_DEFAULT); // Mã hóa mật khẩu trước khi lưu
            // Lấy ID của người dùng vừa được thêm vào
            $last_user_id = $conn->insert_id;
            // Thêm thông tin người dùng vào bảng user
            $sql_user = "INSERT INTO user (Id, Name, Email, Password, major_id, role) VALUES ('$last_user_id', '$hoten', '$masv', $hashed_password, (SELECT Id FROM major WHERE name = '$major_name'), 'student')";
            $result_user = $conn->query($sql_user);

            if ($result_user === TRUE) {
                // Thêm thông tin sinh viên vào bảng student
                $sql_student = "INSERT INTO student (Name, MaSV, lopsinhoat, major, Id) VALUES ('$hoten', '$masv', '$lop', (SELECT Id FROM major WHERE name = '$major_name'), '$last_user_id')";
