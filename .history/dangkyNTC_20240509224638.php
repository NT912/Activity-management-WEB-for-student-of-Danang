...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name_orginization) || empty($email) || empty($matkhau)) {
        echo "<script>alert('Vui lòng điền đầy đủ thông tin.');</script>";
    } else {
        // Kiểm tra xem tên nhà tổ chức đã tồn tại trong cơ sở dữ liệu chưa
        $sql_check = "SELECT * FROM Orginization_information WHERE Name_orginization = '$name_orginization'";
        ...
        if ($result_check->num_rows > 0) {
            ...
            $sql_student = "INSERT INTO Student (Name, email, Password, lopsinhoat, major) VALUES ('$name_orginization', '$email', '$matkhau', '$lop', '$khoa')";
