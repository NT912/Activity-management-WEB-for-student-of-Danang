...
exports.GetallActivity = () => {
  return new Promise((resolve, reject) => {
      const query = `SELECT Id_activity, Id_organization, Name_activity, activity.Description, Date_start, Date_end, Date_start_registe, Date_end_registe, Location, image, Id, Name, avatar FROM activity INNER JOIN organization on organization.Id = activity.Id_organization ORDER BY activity.created_at ASC`;
      db.query(query, (err, res) => {
        if (err) {
          reject(err);
        } else {
